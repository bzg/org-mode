...
- /Org mode buffers/ ::
  ...
  =ID= property for the link[fn:28].  So using this command in Org
...
- {{{kbd(C-c C-l)}}} (~org-insert-link~) ::
  ...
  Insert a link[fn:29].  This prompts for a link to be inserted into
  ...
  a descriptive text[fn:30].  If some text was selected at this time,
  ...
  - /Completion support/ ::
    ...
    specific completion support for some link types[fn:31].  For
...
- {{{kbd(mouse-3)}}} ::
  ...
  Emacs, and internal links to be displayed in another window[fn:32].
...
line number or a search option after a double colon[fn:33].  For
...
Org mode does not maintain TODO lists as separate documents[fn:34].
...
states in the process of working on an item, for example[fn:35]:
...
C-t)}}}[fn:36].  When used several times in succession, it still
...
selection character after each keyword, in parentheses[fn:37].  For
example:
...
remove any TODO keyword from an entry[fn:38].
...
changes known to Org mode[fn:39].
...
TODO item was marked as done.  This can be achieved with[fn:40]
...
a note along with the timestamp, use[fn:41]
...
inserted after the headline as an itemized list, newest first[fn:42].
...
for this is called =LOGBOOK=[fn:43].  You can also overrule the
...
=CANCELED=[fn:44].  The setting for =WAIT= is even more special: the
...
- {{{kbd(S-UP)}}} (~org-priority-up~); {{{kbd(S-DOWN)}}} (~org-priority-down~) ::
  ...
  Increase/decrease the priority of the current headline[fn:45].  Note
...
below a TODO item, with detailed subtasks on the tree[fn:46].  To keep
...
#+vindex: org-list-automatic-rules
Every item in a plain list[fn:47] (see [[*Plain Lists]]) can be made into
...
appears[fn:48].  You have to insert the cookie yourself by typing
...
- {{{kbd(C-c C-c)}}} (~org-toggle-checkbox~) ::
  ...
  remove the current one[fn:49].  With a double prefix argument, set
...
surrounds the entire file.  Use a line like this[fn:50]
...
form---match as well[fn:51].  The list of matches may then become
...
the buffer[fn:52].  You may also globally specify a hard list of tags
...
valid tags with corresponding keys[fn:53].
...
- {{{var(SUMMARY-TYPE)}}} ::
  ...
  are computed from the children[fn:54].
  ...
  | =@min=   | Minimum age[fn:55] (in days/hours/mins/seconds).      |
...
a contributed package[fn:57].  It provides a general API to collect
...
=<2003-09-16 Tue 09:39>= or =<2003-09-16 Tue 12:00-12:30>=[fn:58].
...
- Diary-style expression entries ::
  ...
  package[fn:59].  For example, with optional time:
...
date[fn:60].  If the date has been automatically shifted into the
...
Parallel to the minibuffer prompt, a calendar is popped up[fn:61].
...
is displayed live in the minibuffer[fn:62].
...
- =SCHEDULED= ::
  ...
  The headline is listed under the given date[fn:63].  In addition,
...
=SCHEDULED=, or plain timestamps[fn:67].  In the following example:
...
- {{{kbd(C-c C-x C-i)}}} (~org-clock-in~) ::
  ...
  line displays the current clocking time against it[fn:72].  If the
  task is a repeating one (see [[*Repeated tasks]]), show only the time
  since the last reset of the task[fn:73].  You can exercise more
  ...
  time, or ~auto~ which is the default[fn:74].  Clicking with
...
- =:block= ::
  ...
  | =untilnow=[fn:76]                       | all clocked time ever |
...
- {{{kbd(C-c C-w)}}} (~org-refile~) ::
  ...
  variable ~org-log-refile~[fn:81] is set, a timestamp or a note is
...
- {{{kbd(<)}}} ::
  ...
  #+kindex: < @r{(Agenda dispatcher)}
  Restrict an agenda command to the current buffer[fn:90].  If
...
- {{{kbd(M-x org-agenda a)}}} (~org-agenda-list~) ::
  ...
  argument[fn:92]---like {{{kbd(C-u 2 1 M-x org-agenda a)}}}---you may
...
them, as seen in the following segment of an Org file:[fn:93]
...
- {{{kbd(v R)}}} or short {{{kbd(R)}}} (~org-agenda-clockreport-mode~) ::
  ...
  hidden by agenda filtering[fn:96].  See also the variable
...
- {{{kbd(i)}}} (~org-agenda-diary-entry~) ::
  ...
  file[fn:98], in a way similar to the {{{kbd(i)}}} command in the
...
- {{{kbd(x)}}} ::
  ...
  as a global search for agenda entries planned[fn:100] this week/day.
...
- {{{kbd(TAB)}}} ::
  ...
  a LaTeX fragment[fn:110].  For example, {{{kbd(TAB)}}} expands =fr=
...
Org's outline with stars and no indents can look cluttered for
short documents.  For /book-like/ long documents, the effect is not as
...
with virtual spaces to vertically align with the headline text[fn:147].
...
lign up with the outline headers.  You need these settings:
...
- /Hiding leading stars/ ::
  ...
  get the effect right.

- /Odd levels/ ::

  #+vindex: org-odd-levels-only
  Using stars for only odd levels, 1, 3, 5, ..., can also clean up the
  clutter.  This adds two stars for each level[fn:151].  For Org
...
[fn:163] The actual behavior of the search depends on the value of the
...
[fn:27] If the headline contains a timestamp, it is removed from the
link, which results in a wrong link---you should avoid putting
a timestamp in the headline.

[fn:28] The Org Id library must first be loaded, either through
~org-customize~, by enabling ~id~ in ~org-modules~, or by adding
=(require 'org-id)= in your Emacs init file.

[fn:29] Note that you do not have to use this command to insert
...
[fn:30] After insertion of a stored link, the link will be removed
...
[fn:31] This works if a function has been defined in the ~:complete~
property of a link in ~org-link-parameters~.

[fn:32] See the variable ~org-link-use-indirect-buffer-for-internals~.

[fn:33] For backward compatibility, line numbers can also follow a
single colon.

[fn:34] Of course, you can make a document that contains only long
lists of TODO items, but this is not required.

[fn:35] Changing the variable ~org-todo-keywords~ only becomes
effective after restarting Org mode in a buffer.

[fn:36] This is also true for the {{{kbd(t)}}} command in the agenda
buffer.

[fn:37] All characters are allowed except =@=, =^= and =!=, which have
a special meaning here.

[fn:38] Check also the variable ~org-fast-tag-selection-include-todo~,
...
[fn:39] Org mode parses these lines only when Org mode is activated
after visiting a file.  {{{kbd(C-c C-c)}}} with point in a line
starting with =#+= is simply restarting Org mode for the current
buffer.

[fn:40] The corresponding in-buffer setting is: =#+STARTUP: logdone=.

[fn:41] The corresponding in-buffer setting is: =#+STARTUP:
lognotedone=.

[fn:42] See the variable ~org-log-states-order-reversed~.

[fn:43] Note that the =LOGBOOK= drawer is unfolded when pressing
{{{kbd(SPC)}}} in the agenda to show an entry---use {{{kbd(C-u
SPC)}}} to keep it folded here.

[fn:44] It is possible that Org mode records two timestamps when you
...
[fn:45] See also the option ~org-priority-start-cycle-with-default~.

[fn:46] To keep subtasks out of the global TODO list, see the option
~org-agenda-todo-list-sublevels~.

[fn:47] With the exception of description lists.  But you can allow it
by modifying ~org-list-automatic-rules~ accordingly.

[fn:48] Set the variable ~org-hierarchical-checkbox-statistics~ if you
want such cookies to count all checkboxes below the cookie, not just
those belonging to direct children.

[fn:49] {{{kbd(C-u C-c C-c)}}} on the /first/ item of a list with no
checkbox adds checkboxes to the rest of the list.

[fn:50] As with all these in-buffer settings, pressing {{{kbd(C-c
C-c)}}} activates any changes in the line.

[fn:51] This is only true if the search does not involve more complex
tests including properties (see [[*Property Searches]]).

[fn:52] To extend this default list to all tags used in all agenda
files (see [[*Agenda Views]]), customize the variable
~org-complete-tags-always-offer-all-agenda-tags~.

[fn:53] Keys are automatically assigned to tags that have no
configured keys.

[fn:54] If more than one summary type applies to the same property,
the parent values are computed according to the first of them.

[fn:55] An age can be defined as a duration, using units defined in
~org-duration-units~, e.g., =3d 1h=.  If any value in the column is as
such, the summary is also expressed as a duration.

[fn:56] Please note that the =COLUMNS= definition must be on a single
line; it is wrapped here only because of formatting constraints.

[fn:57] Contributed packages are not part of Emacs, but are
distributed with the main distribution of Org---visit
[[https://orgmode.org]].

[fn:58] The Org date format is inspired by the standard ISO 8601
...
[fn:59] When working with the standard diary expression functions, you
...
[fn:60] See the variable ~org-read-date-prefer-future~.  You may set
that variable to the symbol ~time~ to even make a time before now
shift the date to tomorrow.

[fn:61] If you do not need/want the calendar, configure the variable
~org-popup-calendar-for-date-prompt~.

[fn:62] If you find this distracting, turn off the display with
~org-read-date-display-live~.

[fn:63] It will still be listed on that date after it has been marked
as done.  If you do not like this, set the variable
~org-agenda-skip-scheduled-if-done~.

[fn:64] The =SCHEDULED= and =DEADLINE= dates are inserted on the line
right below the headline.  Do not put any text between this line and
the headline.

[fn:65] Note the corresponding =STARTUP= options =logredeadline=,
=lognoteredeadline=, and =nologredeadline=.

[fn:66] Note the corresponding =STARTUP= options =logreschedule=,
=lognotereschedule=, and =nologreschedule=.

[fn:67] Org does not repeat inactive timestamps, however.  See
[[*Timestamps]].
...
[fn:76] When using ~:step~, ~untilnow~ starts from the beginning of
...
[fn:81] Note the corresponding =STARTUP= options =logrefile=,
...
[fn:101] For HTML you need to install Hrvoje Nikšić's =htmlize.el=
