...
#+findex: org-submit-feature-request
For feature requests and suggestions, you can either directly send
a message to the mailing list, or use
: M-x org-submit-feature-request <RET>

#+findex: org-submit-patch
The patch submissions can also be prepared from inside Emacs, using
: M-x org-submit-patch
You can find more contribution instructions on [[https://orgmode.org/worg/org-contribute.html][our wiki]].
...
3. The TODO has a scheduled date, usually with a =.+= style repeat
   interval[fn::all the repeater intervals except hours (=h=) are
   supported.].  A =++= style may be appropriate for habits with time
...
#+vindex: org-checkbox-children-only-statistics
...
appears[fn:: Set the variable ~org-checkbox-children-only-statistics~
...
For help editing text with hidden emphasis marks, see [[*Dealing with Hidden Text]].
...
For help editing text with hidden sub- and superscript markers, see
...
- {{{kbd(C-c C-x C-v)}}} (~org-link-preview~) ::
  ...
  toggle previews in the whole buffer instead.  You can also enable
  previewing links with description globally by customizing
...
The macro arguments cannot contain =}}}= inside.  In other words,
...
~html-preamble~.  Use it preferably to set values like ~t~, ~nil~, or
a function.  To set your preamble as a string, use the keyword
~#+HTML_PREAMBLE~.  You can use it multiple times to set a multi-line
string.
...
- ~org-html-table-use-header-tags-for-first-column~ ::
  ...
  : #+OPTIONS: html-table-use-header-tags-for-first-column:t
...
~org-html-inline-images~ to ~nil~ to have no in-lined images and only
...
~#+HTML_MATHJAX_TEMPLATE~.  This keyword can be set multiple times in
the buffer, to set a multi-line template.
...
- =LATEX_DOC_METADATA= ::
  ...
  #+cindex: @samp{LATEX_DOC_METADATA}, keyword
  #+vindex: org-latex-doc-metadata
  Arbitrary lines with document metadata. When you set
  ~org-latex-doc-metadata~ or you use ~#+LATEX_DOC_METADATA~, the
  value will be inserted as =\DocumentMetadata{<value>}=
  at the beginning of the LaTeX preamble.
...
- If ~org-inside~ is enabled (see [[*Dealing with Hidden Text]]) and
  point is inside text with hidden contents, toggle hiding.
...
- =#+STARTUP:= ::
  ...
  brackets.  For example, =[[https://orgmode.org][Org Website]]= is, by
  default, displayed as "Org Website", hiding the link itself and just
  displaying its description.  Alternatively, the links can be
  displayed in full.  The corresponding variable is
  ~org-link-descriptive~.  For help editing text with hidden links and
  brackets, see [[*Dealing with Hidden Text]].
...
** Dealing with Hidden Text
:PROPERTIES:
:DESCRIPTION: Using ~org-inside~ to help with editing text with hidden contents.
:END:
#+kindex: C-c C-c
#+cindex: org-inside, revealing hidden components, hidden text components

In Org, entity types including [[*Emphasis and Monospace][emphasized
text]], [[*Link Format][links]], and [[*Subscripts and
Superscripts][super & subscript]] can contain hidden components
(markers, brackets, link URL, etc.).  When the cursor is adjacent to
such hidden text, the position can be ambiguous: at the same apparent
cursor position, point can be either /inside/ or /outside/ the hidden
region.  This can make it difficult to know where edits will go.

To alleviate this problem, activate the mode ~org-inside~.  When this
minor mode is active in an Org buffer, the appearance is changed when
point is "inside" such an entity, to make it clear where you are.
Appearance changes can include entity face (e.g. a subtle background
color), cursor type (e.g. bar instead of box), and even (automatic)
un-hiding of the hidden contents.

#+vindex: org-inside-appearance
The appearance changes to apply when inside entities with hidden
contents can be configured using the custom variable
~org-inside-appearance~.

#+vindex: org-inside-unhide-delay
The visibility of hidden text can be toggled using the {{{kbd(C-c
C-c)}}} key (see [[*The Very Busy {{{kbd(C-c C-c)}}} Key]]) when
inside an entity with hidden contents.  Hidden text can also be made
visible automatically upon entry, with an optional delay (see
~org-inside-unhide-delay~).  Text is always re-hidden on exit.
~org-inside~ supports nested entities, applying the /face/ appearance
changes (if configured) only to the /innermost/ element.

Note that certain ~org-inside~ features require Emacs v31 or later.
** Code Evaluation and Security Issues
:PROPERTIES:
